...
app.post("/participants", async (req, res) => {
    ...
    try {
        ...
        await db
            ...
            .insertOne({ name: assert.equal(stripHtml(participant.name).result,participant.name).trim(), lastStatus: entry })
        await db
            .collection("messages")
            .insertOne({
                from: assert.equal(stripHtml(participant.name).result,participant.name).trim(),
                to: 'Todos',
                text: 'entra na sala...',
                type: 'status',
                time: time
...
app.post("/messages", async (req, res) => {
    ...
    try {
        ...
        await db
            .collection("messages")
            .insertOne({ from: from, to: message.to, text: assert.equal(stripHtml(message.text).result,message.text).trim(), type: message.type, time: time })
        res.sendStatus(201)
    } catch (error) {
        console.log(error)
        res.sendStatus(500)
...
app.post("/status", async (req, res) => {
    try {
        ...
        await db
            .collection('participants')
            .updateOne({ name: assert.equal(stripHtml(user).result,user).trim() }, { $set: { lastStatus: entry } })
